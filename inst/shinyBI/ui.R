             ...
             fluidPage(
               sidebarLayout(
                 sidebarPanel(width = 3,
                              ...
                              selectInput("aggr_fun_input", label="aggregate function", choices=c("sum","mean","median","min","max","length"), selected="sum", multiple=FALSE)), # list("sum"=sum,"avg"=mean,"median"=median,"min"=min,"max"=max,"count"=length,"count distinct"=uniqueN)
                 mainPanel(width = 9,
                           dataTableOutput("dt"))
               )
             ))
