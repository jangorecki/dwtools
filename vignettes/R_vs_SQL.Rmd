...
```{r r_init, echo=FALSE}
# TO DO
# [x] read data - fread - silent
# [x] remove base tests due to very long time of execution: group, distinct, union
# [x] optimize data population for DB to speedup full test
# [ ] optimize data.equal.data.table
# [ ] add postgres installation to ec2 script
...
Currently supported benchmark volumes are 1e3, 1e5, 1e6, 1e7, 1e8.
```{r N}
N = 1e6
```
...
# filter sample - valid for dw.populate(S=1) and N in 1e3, 1e5, 1e6, 1e7, 1e8:
whereList <- function(N){
  ...
  if(N==1e3){
    ...
    stop("filtering samples were prepared for `N %in% c(1e3, 1e5, 1e6, 1e7, 1e8)`. You need to extend the section *where* to for valid filtering samples for the data.")
  }
  W
}
```
