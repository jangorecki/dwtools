...
r0 = timing(DF2 <- mutate(DF2, value = ifelse(cust_code %in% upd_cust_code, 0, value)),
...
# dplyr
DF = as.data.frame(DT)
r0 <- timing(DF[,-6] %>% group_by(geog_code,curr_code) %>% summarise(value=sum(value)) %>% spread(geog_code, value) %>% group_by(curr_code) %>% summarise_each(funs(sum(.,na.rm=TRUE))), # tidyr #41 issue workaround?
             N, paste("group_by-summarise-spread-group_by-summarise_each","pivot","no_index","tidyr",sep=";"))
rm(DF)
...
```
* Open RStudio in the your local web browser: `http://amazon.instance.ip:8787/`
* Install R packages:
```{r ec2_install_pkgs, echo=TRUE, eval=FALSE}
install.packages(c("RCurl","devtools","knitr","microbenchmark","rmarkdown","DBI","dplyr","tidyr","RSQLite"), repos="http://cran.stat.ucla.edu")
library(devtools)
