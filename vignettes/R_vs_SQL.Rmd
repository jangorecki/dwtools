...
# TO DO
# [ ] cross apply dplyr, data.table, postgres
...
```{r cross_apply_show, echo=FALSE, results='asis', eval=FALSE}
#invisible(showtiming(last=last)[,print_expr(.SD)])
```
...
r = lapply(db_test, function(db_t){ # timing of each sql per each db_test
  ...
  stopifnot(data.equal.data.table(rDB,r0))
  invisible()
})
...
DT = copy(X$SALES)
REF = copy(X$PRODUCT)
REF[,c("prod_name","prod_group_code","prod_group_name","prod_family_code", "prod_family_name"):=NULL][,prod_value_mp := rnorm(.N,1,sd=0.10)]
REF_back = copy(REF)
...
# db_test[!(db_test %like% "sqlite")]
# try(db("DROP TABLE db", db_test[!(db_test %like% "sqlite")]),silent = TRUE)
# db(DT,"db",db_test[!(db_test %like% "sqlite")])
# r = lapply(db_test[!(db_test %like% "sqlite")], function(db_t){ # timing of each sql per each db_test
#   r0 = eval(bquote(timing(db(paste0("SELECT * FROM db ORDER BY ",paste(names(DT),collapse=',')),.(db_t)),
#                           N,paste("db","pivot","no_index",db_t,sep=";"))))
#   stopifnot(data.equal.data.table(r1,r0))
#   invisible()
# })
non_idx_n <- 1 + 1
...
```
ssh -i auth_key.pem ubuntu@amazon.instance.ip
...
# RStudio Server # port 8787 - open only for ip
sudo apt-get -y install gdebi-core libapparmor1
# use the latest link from: http://www.rstudio.com/products/rstudio/download-server/
wget http://download2.rstudio.org/rstudio-server-0.98.1091-amd64.deb
sudo gdebi -q -n rstudio-server-0.98.1091-amd64.deb
# add RStudio user
sudo useradd -m -p $(openssl passwd -1 rstudio_user_pass) rstudio_user

# open in client web browser
http://amazon.instance.ip:8787/
# log in to RStudio
install.packages(c("RCurl","devtools","knitr","microbenchmark","rmarkdown","DBI","dplyr","tidyr","RSQLite"), repos="http://cran.stat.ucla.edu")

# install_github, might failed due to github calls limit on ec2, in such case git clone manually from github
library(devtools)
