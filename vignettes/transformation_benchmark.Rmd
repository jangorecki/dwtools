...
DT = dw.populate(0,scenario="fact") # 0 nrow DT template to catch names
# Define indices
Idx = list(
  1:3,
  c(1,3,5),
...
DT = dw.populate(N, S=1, scenario="fact")
r1 = eval(bquote(timing(DT[,c("grp_amount","grp_value") := lapply(.SD,sum), by=.(names(W[[1]])), .SDcols=c("amount","value")],
                        N, paste("[.data.table","partition_aggr1","no_index","data.table",sep=";"))))
DT = dw.populate(N, S=1, scenario="fact")
r2 = eval(bquote(timing(DT[,c("grp_amount","grp_value") := lapply(.SD,sum), by=.(names(W[[2]])), .SDcols=c("amount","value")],
                        N, paste("[.data.table","partition_aggr2","no_index","data.table",sep=";"))))
DT = dw.populate(N, S=1, scenario="fact")
r3 = eval(bquote(timing(DT[,c("grp_amount","grp_value") := lapply(.SD,sum), by=.(names(W[[3]])), .SDcols=c("amount","value")],
                        N, paste("[.data.table","partition_aggr3","no_index","data.table",sep=";"))))

DF = as.data.frame(dw.populate(N, S=1, scenario="fact"))
r0 = eval(bquote(timing(DF %>% group_by(cust_code, prod_code, geog_code) %>% mutate_each(funs(sum), grp_amount=amount,grp_value=value),
                        N, paste("select-group_by-mutate_each","partition_aggr1","no_index","dplyr",sep=";"))))
stopifnot(data.equal.data.table(setDT(r1),setDT(as.data.frame(r0))))
r0 = eval(bquote(timing(DF %>% group_by(cust_code, geog_code, curr_code) %>% mutate_each(funs(sum), grp_amount=amount,grp_value=value),
                        N, paste("select-group_by-mutate_each","partition_aggr2","no_index","dplyr",sep=";"))))
stopifnot(data.equal.data.table(setDT(r2),setDT(as.data.frame(r0))))
r0 = eval(bquote(timing(DF %>% group_by(prod_code, geog_code) %>% mutate_each(funs(sum), rp_amount=amount,grp_value=value),
...
DT = dw.populate(N, S=1, scenario="fact")
setkeyv(DT, Idx[[1]])
r0 = eval(bquote(timing(DT[,c("grp_amount","grp_value") := lapply(.SD, sum), by = key(DT), .SDcols=c("amount","value")],
...
df = as.data.frame(FT)
jn = as.data.frame(JN)
r1 = eval(bquote(timing(merge(df, jn, by = "prod_code",all = FALSE),
                        N, paste("merge","inner_join","no_index","base",sep=";"))))
r0 = eval(bquote(timing(df %>% inner_join(jn, by="prod_code"),
...
dt <- copy(FT)
jn <- copy(JN)
r0 = eval(bquote(timing(setkeyv(jn,"prod_code")[setcolorder(dt,c("prod_code",names(dt)[names(dt)!="prod_code"])),nomatch=0],
...
r0 = eval(bquote(timing(dt[jn,nomatch=0],
...
df = as.data.frame(FT)
jn = as.data.frame(JN)
r1 = eval(bquote(timing(merge(df, jn, by = "prod_code",all.x = TRUE, sort=FALSE),
                        N, paste("merge","left_join","no_index","base",sep=";"))))
r0 = eval(bquote(timing(df %>% left_join(jn, by="prod_code"),
...
dt <- copy(FT)
jn <- copy(JN)
r0 = eval(bquote(timing(setkeyv(jn,"prod_code")[setcolorder(dt,c("prod_code",names(dt)[names(dt)!="prod_code"])),nomatch=NA],
...
setkeyv(dt,"prod_code")
setkeyv(jn,"prod_code")
r0 = eval(bquote(timing(jn[dt,nomatch=NA],
...
r1 = timing(merge(x = DF1, y = DF2, by = NULL), N, paste("merge","cross_join","no_index","base",sep=";"))
rm(DF1,DF2)

r0 = timing(CJ.dt(DT1,DT2), N, paste("[.data.table","cross_join","no_index","data.table",sep=";"))
