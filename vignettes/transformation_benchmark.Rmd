...
trunc_expr <- 48L # truncate expression after n chars
```

## Timing

Check timing speed and precision efficiency: microbenchmark (1e-9) vs. system.time (1e-3).
```{r timing_fun}
options("dwtools.timing.nano"=TRUE) # requires microbenchmark
purge.timing()
system.time(
  sapply(seq.int(1e1), function(i){
      timing(Sys.sleep(0.0001))
  })
)
options("dwtools.timing.nano"=FALSE)
system.time(
  sapply(seq.int(1e1), function(i){
      timing(Sys.sleep(0.0001))
  })
)
#kable(get.timing()[c(1L,length(elapsed))])
purge.timing()
options("dwtools.timing.nano"=TRUE) # will leave it on
```
...
Twrite <- get.timing(trunc_expr,last=2+length(db_test))[,list(expr,elapsed=round(elapsed,6),tag)]
kable(Twrite)
```
...
rm(r1, r4, r)
Tread <- get.timing(trunc_expr,last=3+length(db_test))[,list(expr,elapsed=round(elapsed,6),tag)]
kable(Tread)
```
...
r = lapply(db_test, function(db_t){ # timing of 4 sqls at once per each db_test
  eval(bquote(timing(db(create_index_sql,.(db_t)),N,paste("db","create_index","indexN_any",db_t,sep=";"))))
})
Tcreate_index <- get.timing(trunc_expr,last=3+length(db_test))[,list(expr,elapsed=round(elapsed,6),tag)]
kable(Tcreate_index)
```
...
Twhere <- get.timing(trunc_expr,last=(3+length(db_test))*3)[,list(expr,elapsed=round(elapsed,6),tag)]
print("SELECT WHERE no index")
kable(Twhere)
...
r = lapply(db_test, function(db_t){ # timing of each sql per each db_test
  p = lapply(1:length(select_sql), function(i){
    r0 = eval(bquote(timing(db(.(select_sql[i]),.(db_t)),N,paste("db",paste0("where",i),"indexN_any",db_t,sep=";"))))
    stopifnot(data.equal.data.table({if(i==1L) r1 else if(i==2L) r2 else if(i==3L) r3 else stop("three filters defined")},r0))
    invisible()
  })
})
...
Twhere_index <- get.timing(trunc_expr,last=(2+length(db_test))*3)[,list(expr,elapsed=round(elapsed,6),tag)]
print("SELECT WHERE index")
kable(Twhere_index)
```
...
DF = as.data.frame(dw.populate(N, S=1, scenario="fact"))
r1 = eval(bquote(timing(aggregate(DF[c("amount","value")], by=as.list(DF[c("cust_code","prod_code","geog_code")]), FUN=sum),
                        N, paste("aggregate","group1","no_index","base",sep=";"))))
r2 = eval(bquote(timing(aggregate(DF[c("amount","value")], by=as.list(DF[c("cust_code","geog_code","curr_code")]), FUN=sum),
                        N, paste("aggregate","group2","no_index","base",sep=";"))))
r3 = eval(bquote(timing(aggregate(DF[c("amount","value")], by=as.list(DF[c("prod_code","geog_code")]), FUN=sum),
                        N, paste("aggregate","group3","no_index","base",sep=";"))))

DF = as.data.frame(dw.populate(N, S=1, scenario="fact"))
r0 = eval(bquote(timing(DF %>% select(cust_code, prod_code, geog_code, amount, value) %>% group_by(cust_code, prod_code, geog_code) %>% summarise_each(funs(sum)),
...
r0 = eval(bquote(timing(DF %>% select(cust_code, geog_code, curr_code, amount, value) %>% group_by(cust_code, geog_code, curr_code) %>% summarise_each(funs(sum)),
...
r0 = eval(bquote(timing(DF %>% select(prod_code, geog_code, amount, value) %>% group_by(prod_code, geog_code) %>% summarise_each(funs(sum)),
...
DT = dw.populate(N, S=1, scenario="fact")
r0 = eval(bquote(timing(DT[,lapply(.SD,sum), by=c("cust_code","prod_code","geog_code"), .SDcols=c("amount","value")],
                        N, paste("[.data.table","group1","no_index","data.table",sep=";"))))
stopifnot(data.equal.data.table(r1,r0))
r0 = eval(bquote(timing(DT[,lapply(.SD,sum), by=c("cust_code","geog_code","curr_code"), .SDcols=c("amount","value")],
                        N, paste("[.data.table","group2","no_index","data.table",sep=";"))))
stopifnot(data.equal.data.table(r2,r0))
r0 = eval(bquote(timing(DT[,lapply(.SD,sum), by=c("prod_code","geog_code"), .SDcols=c("amount","value")],
...
Tgroup <- get.timing(trunc_expr,last=(3+length(db_test))*3)[,list(expr,elapsed=round(elapsed,6),tag)]
print("GROUP BY no index")
kable(Tgroup)
...
DT = dw.populate(N, S=1, scenario="fact")
setkeyv(DT, Idx[[1]])
r0 = eval(bquote(timing(DT[,lapply(.SD, sum), by = .(names(W[["1"]])), .SDcols=c("amount","value")],
                        N, paste("[.data.table","group1","index1_any","data.table",sep=";"))))
stopifnot(data.equal.data.table(r1,r0))
r0 = eval(bquote(timing(DT[,lapply(.SD, sum), by = .(names(W[["2"]])), .SDcols=c("amount","value")],
                        N, paste("[.data.table","group2","index1_any","data.table",sep=";"))))
stopifnot(data.equal.data.table(r2,r0))
r0 = eval(bquote(timing(DT[,lapply(.SD, sum), by = .(names(W[["3"]])), .SDcols=c("amount","value")],
                        N, paste("[.data.table","group3","index1_any","data.table",sep=";"))))
stopifnot(data.equal.data.table(r3,r0))
...
r = lapply(db_test, function(db_t){ # timing of each sql per each db_test
  p = lapply(1:length(select_sql), function(i){
    r0 = eval(bquote(timing(db(.(select_sql[i]),.(db_t)),N,paste("db",paste0("group",i),"indexN_any",db_t,sep=";"))))
    stopifnot(data.equal.data.table({if(i==1L) r1 else if(i==2L) r2 else if(i==3L) r3 else stop("three filters defined")},r0))
    invisible()
  })
})
...
Tgroup_index <- get.timing(trunc_expr,last=(1+length(db_test))*3)[,list(expr,elapsed=round(elapsed,6),tag)]
print("GROUP BY index")
kable(Tgroup_index)
```

* data.table solutions used only single key set during the tests, this has to reflect support for concurency access to same data.table.
