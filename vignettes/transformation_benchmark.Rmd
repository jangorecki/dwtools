...
```{r distinct_show, echo=FALSE, results='asis'}
invisible(showtiming(last=last)[,print_expr(.SD)])
```
...
r1 <-timing(unique(rbind(DF1,DF2)),
            N, paste("unique","union","no_index","base",sep=";"))
# dt
...
```{r union_show, echo=FALSE, results='asis'}
invisible(showtiming(last=last)[,print_expr(.SD)])
```

## union all

```{r union_all, echo=FALSE}
DT = copy(X$SALES)
# DT = rbindlist(list(DT,DT))
DT1 = DT[1:(nrow(DT)/2)]
DT2 = DT[((nrow(DT)/2)+1):nrow(DT)]
...
```{r union_all_show, echo=FALSE, results='asis'}
invisible(showtiming(last=last)[,print_expr(.SD)])
```
...
```{r minus_show, echo=FALSE, results='asis'}
invisible(showtiming(last=last)[,print_expr(.SD)])
```
...
# base
# r1 <-timing(rbind(DF1,DF2),
#             N, paste("?","intersect","no_index","base",sep=";"))
# dt
r1 <- timing(setkeyv(DT1,names(DT1))[setkeyv(DT2,names(DT2)),.SD,nomatch=0L,.SDcols=c(names(DT1))],
             N, paste("setkeyv-[.data.table","intersect","no_index","data.table",sep=";"))
#stopifnot(data.equal.data.table(setDT(r1),r0))
# dplyr
DT1 = DT[i1]
DT2 = DT[i2]
...
non_idx_n <- 0 + 1 + 1 + 1*length(db_test)
...
```{r intersect_show, echo=FALSE, results='asis'}
invisible(showtiming(last=last)[,print_expr(.SD)])
```
