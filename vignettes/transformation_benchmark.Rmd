...
pkgsVersion(pkgs, c(home = .libPaths()[1]))
```
...
timing(write.table(DT,"benchmark.csv",sep=",",row.names=FALSE,quote=FALSE), N, paste("write","csv","base",sep=";"))

if(file.exists("benchmark.rds")) file.remove("benchmark.rds")
timing(saveRDS(DT,"benchmark.rds"), N, paste("write","rds","base",sep=";"))
...
r1 = timing(fread("benchmark.csv")[,time_code:=as.Date(time_code)],
            N, paste("read","csv","data.table",sep=";"))
...
r2 = timing(read.table("benchmark.csv",header=TRUE,sep=",",quote="",stringsAsFactors=FALSE,comment.char="",nrows=N,colClasses=unname(DT[,sapply(.SD,class)])),
            N, paste("read","csv","base",sep=";"))
stopifnot(data.equal.data.table(r1,setDT(r2))); rm(r2); file.remove("benchmark.csv")

if(file.exists("benchmark.rds")) file.remove("benchmark.rds")
saveRDS(DT,"benchmark.rds")
r3 = timing(readRDS("benchmark.rds"),
            N, paste("read","rds","base",sep=";"))
stopifnot(data.equal.data.table(r1,r3)); rm(r3); file.remove("benchmark.rds")
...
DT = dw.populate(N, S=1, scenario="fact")
r = timing(idxv(DT, Idx),N,paste("create_index","indexN_where","dwtools",sep=";"))
DT = dw.populate(N, S=1, scenario="fact")
r = timing(idxv(DT, Idx, grp=TRUE),N,paste("create_index","indexN_group","dwtools",sep=";"))
DT = dw.populate(N, S=1, scenario="fact")
r = eval(bquote(timing(setkeyv(DT, .(Idx[[1]])),N,paste("create_index","index1_any","data.table",sep=";"))))
DT = dw.populate(N, S=1, scenario="fact")
try(db("DROP TABLE db", db_test),silent = TRUE)
db(DT,"db",db_test)
...
r = lapply(db_test, function(db_t){ # timing of 4 sqls at once per each db_test
  eval(bquote(timing(db(create_index_sql,.(db_t)),NA_integer_,paste("create_index","indexN_any",db_t,sep=";"))))
})
```
...
```{r where, echo=FALSE}
DT = dw.populate(N, S=1, scenario="fact")
# filter sample - valid for dw.populate(S=1) and N in 1e3, 1e5, 1e6:
W <- list()
if(N==1e3){
  ...
  stop("filtering samples were prepared for `N %in% c(1e3, 1e5, 1e6)`. You need to extend the section *where* to for valid filtering samples for the data, use S=1: `dw.populate(N, S=1, scenario='fact')`.")
}
...
r1 = eval(bquote(timing(DF[DF$cust_code%in%.(W[["1"]][["cust_code"]]) & DF$prod_code%in%.(W[["1"]][["prod_code"]]) & DF$geog_code%in%.(W[["1"]][["geog_code"]]),],
                        N, paste("where1","no_index","base",sep=";"))))
...
r0 = eval(bquote(timing(DT[cust_code%in%.(W[["1"]][["cust_code"]]) & prod_code%in%.(W[["1"]][["prod_code"]]) & geog_code%in%.(W[["1"]][["geog_code"]])],
                        N, paste("where1","no_index","data.table",sep=";"))))
...
r = lapply(db_test, function(db_t){ # timing of each sql per each db_test
  p = lapply(1:length(select_sql), function(i){
    r0 = eval(bquote(timing(db(.(select_sql[i]),.(db_t)),NA_integer_,paste(paste0("where",i),"db",db_t,sep=";"))))
...
r0 = eval(bquote(timing(DT[CJ(.(W[["1"]][["cust_code"]]),.(W[["1"]][["prod_code"]]),.(W[["1"]][["geog_code"]])),nomatch=0L],
                        N, paste("where1","index1_any","data.table",sep=";"))))
stopifnot(data.equal.data.table(r1,r0))
r0 = eval(bquote(timing(DT[cust_code%in%.(W[["2"]][["cust_code"]]) & geog_code%in%.(W[["2"]][["geog_code"]]) & curr_code%in%.(W[["2"]][["curr_code"]])],
                        N, paste("where2","index1_any","data.table",sep=";"))))
stopifnot(data.equal.data.table(r2,r0))
r0 = eval(bquote(timing(DT[prod_code%in%.(W[["3"]][["prod_code"]]) & geog_code%in%.(W[["3"]][["geog_code"]])],
                        N, paste("where3","index1_any","data.table",sep=";"))))
stopifnot(data.equal.data.table(r3,r0))

DT = dw.populate(N, S=1, scenario="fact")
IDX = idxv(DT, Idx)
r0 = eval(bquote(timing(DT[CJI(IDX,.(W[["1"]][["cust_code"]]),.(W[["1"]][["prod_code"]]),.(W[["1"]][["geog_code"]]))],
                        N, paste("where1","indexN_where","dwtools",sep=";"))))
stopifnot(data.equal.data.table(r1,r0))
r0 = eval(bquote(timing(DT[CJI(IDX,.(W[["2"]][["cust_code"]]),TRUE,.(W[["2"]][["geog_code"]]),TRUE,.(W[["2"]][["curr_code"]]))],
                        N, paste("where2","indexN_where","dwtools",sep=";"))))
stopifnot(data.equal.data.table(r2,r0))
r0 = eval(bquote(timing(DT[CJI(IDX,TRUE,.(W[["3"]][["prod_code"]]),.(W[["3"]][["geog_code"]]))],
                        N, paste("where3","indexN_where","dwtools",sep=";"))))
stopifnot(data.equal.data.table(r3,r0))
...
r = lapply(db_test, function(db_t){ # timing of each sql per each db_test
  p = lapply(1:length(select_sql), function(i){
    r0 = eval(bquote(timing(db(.(select_sql[i]),.(db_t)),NA_integer_,paste(paste0("where",i),"indexN",db_t,sep=";"))))
    stopifnot(data.equal.data.table({if(i==1L) r1 else if(i==2L) r2 else if(i==3L) r3 else stop("three filters defined")},r0))
    invisible()
  })
})
...
DF = as.data.frame(dw.populate(N, S=1, scenario="fact"))
r1 = eval(bquote(timing(DF[DF$cust_code & DF$prod_code & DF$geog_code,],
